DELIMITER //
CREATE PROCEDURE sp_stats_registro_notas(
...
DELIMITER //
CREATE PROCEDURE sp_get_activity_grades(
    IN p_id_activity INT
)
BEGIN
...
DELIMITER //
CREATE PROCEDURE sp_save_student_grade(
    IN p_id_student INT,
    IN p_id_activity INT,
    IN p_value VARCHAR(5)
)
BEGIN
    INSERT INTO grades (id_student, id_activity, value, created_at, updated_at)
    VALUES (p_id_student, p_id_activity, TRIM(p_value), CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    ON DUPLICATE KEY UPDATE value = TRIM(p_value), updated_at = CURRENT_TIMESTAMP;
END //
DELIMITER ;
